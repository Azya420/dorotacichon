...
function updateStatus(pageIndex = 0) {
  ...
  if (pageNumber === 1) pageStatus.textContent = `Cover · page 1 of ${pdfPageCount}`;
  else if (pageNumber === pdfPageCount) pageStatus.textContent = `Last page · page ${pdfPageCount} of ${pdfPageCount}`;
  else {
...
function loadPageFlipLibrary() {
  return new Promise((resolve, reject) => {
    if (window.St?.PageFlip) return resolve();
...
async function renderRemainingPages(pages) {
  for (let i = 3; i < pages.length; i += 1) {
    ...
    if (pageFlip) pageFlip.updateFromHtml(pages);
  }
  updateStatus(pageFlip?.getCurrentPageIndex?.() || 0);
}

function nextPage() { if (pageFlip && isBookReady) pageFlip.flipNext('bottom'); }
function prevPage() { if (pageFlip && isBookReady) pageFlip.flipPrev('bottom'); }
function firstPage() { if (pageFlip && isBookReady) pageFlip.turnToPage(0); }
...
function openPdfPage(pdfPageNumber) { jumpToPage(pdfPageNumber); }
  ...
  projectGrid.innerHTML = projects.map((project, index) => `
    <article class="project-card">
      <button type="button" class="project-open-area" data-pdf-page="${project.startPdfPage}" aria-label="Open ${project.title} in portfolio">
        <span class="project-thumb"><img data-project-thumb-index="${index}" alt="Preview image for ${project.title}" /></span>
...
async function loadThumbnails() {
  ...
  const pdfThumbs = [...document.querySelectorAll('img[data-pdf-thumb]')];
  await Promise.all(pdfThumbs.map(async (img) => {
    img.src = await renderPdfPageImage(Number(img.dataset.pdfThumb), 900);
  }));
}
...
function initNavigation() {
  ...
  window.addEventListener('popstate', () => showView(window.location.hash.replace('#', '') || 'book', false));
...
pageInput.addEventListener('keydown', (event) => { if (event.key === 'Enter') jumpToPage(pageInput.value); });
